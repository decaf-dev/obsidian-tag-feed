...
export class BlockRenderer extends MarkdownRenderChild {
	...
	async onload() {
		const { tag, modifiedTime } = SourceParser.parseSource(this.source);
		if (tag === null) {
			this.containerEl.createEl("div", { text: "No tag specified" });
			return;
		} else if (modifiedTime === -1) {
		...
		for (let i = 0; i < files.length; i++) {
			...
			//Only display blocks that will within the time range
			if (modifiedTime !== null) {
				if (Date.now() - file.stat.mtime > modifiedTime) {
					continue;
				}
			}
			...
			const blocks = findBlocks(tag, lines);
